...
if __name__ == "__main__":
    ...
    """ Theorem 3. Verify 3 rounds are 2^{-23.42} close. """

    parallel = 10

    for i in tqdm.tqdm(range(0, F, parallel)):
        # run 10 layouts in parallel and verify that 3 rounds
        # are at least 2^{-23.42}-close to pairwise
        processes = []
        
        for r in range(parallel):
            if i + r < F:
                processes.append(Process(target=verify_close,
                                        args=(i+r, 3, CC_powers[1], 23.42)))
                processes[-1].start()
